...
int IndexElement2(int[] array, int N)
{
    int IndCount = 0;
    for (int i = 0; i < array.Length; i++) 
        if(array[i] == N)
        {
            IndCount++;
            Console.WriteLine($"Index of element {N} = [{i}]");
        }
        return IndCount;
}
...
Console.WriteLine("|~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~|");
Console.WriteLine($"Первый индекс заданного элемента из массива: [{IndexElement(array, N)}]");
Console.WriteLine($"Количество индексов с заданным числом: {IndexElement2(array, N)}");
Console.WriteLine("|~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~|");
